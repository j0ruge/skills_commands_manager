...
| **pdf-generation** | 1.2.0 | Development | PDF generation design toolkit — analyzes reference templates, recommends libraries (pdfmake/pdf-lib/PDFKit/Puppeteer/@react-pdf), designs modular section architecture with conditional columns, auto-generated observations and revision control. **v1.2.0** adds three production-proven pdfmake pitfalls: cell padding NOT discounted from `widths` (last column silently cuts on A4 with 8+ cols, most painful pdfmake gotcha); Roboto bundled has broken `fi`/`fl`/`ffi` ligatures (`fiscal`→`fscal`, `fixture`→`fxture`); `addFonts()` silently rejects AFM (errors 500 on `getBuffer`). Plus Phase 6 Visual Verification (NON-NEGOTIABLE) — render bugs only surface in the rendered PDF, never in automated tests |
