...
[![Plugins](https://img.shields.io/badge/plugins-20-blue?style=flat-square)](#available-plugins)
...
```bash
/plugin install codereview    # or: ansible-docker-backup-restore, cicd, criar-prd, criar-task, criar-techspec, ddd, deploy, dev-script, dotnet-wpf, executar-task, kaizen-software, pdf-generation, release, retrofit-skill, statusline, ticket, whisper-preprocess, wsl-windows-onboarding, zitadel-idp
```
...
|--------|---------|----------|-------------|
| [**criar-prd**](#criar-prd) | 0.1.0 | Development | Cria PRDs enxutos a partir de uma descrição de funcionalidade, com até 5 perguntas de clarificação, template `templates/prd-template.md` e pesquisa web só quando houver dependência externa ou informação atual. |
| [**criar-techspec**](#criar-techspec) | 0.1.0 | Architecture | Cria Tech Specs objetivas a partir de um PRD local, lendo o template do projeto, regras relevantes e apenas os arquivos necessários para definir uma solução implementável. |
...
| Command | Description |
|---------|-------------|
| `/criar-prd <funcionalidade>` | Faz até 5 perguntas de clarificação, usa `templates/prd-template.md` e salva `tasks/prd-[slug]/prd.md` |
...
| Command | Description |
|---------|-------------|
| `/criar-techspec <slug>` | Lê `tasks/prd-[slug]/prd.md`, segue `templates/techspec-template.md` e salva `tasks/prd-[slug]/techspec.md` |

**Guardrails:** consulta regras do projeto conforme domínio, usa `rg` e `graphify` para impacto quando útil, e pergunta só quando houver bloqueio real.
...
| Command | Description |
|---------|-------------|
| `/criar-task <slug>` | Propõe até 10 tarefas high-level, aguarda aprovação e então gera `tasks.md` + `N_task.md` |
...
| Command | Description |
|---------|-------------|
| `/executar-task <slug> [num]` | Seleciona a task indicada ou a primeira desbloqueada, implementa, valida e atualiza `tasks.md` |
