...
| **unlovable** | ✓ | ✓ | Skill — works on both platforms |
...
```bash
/plugin install codereview    # or: ansible-docker-backup-restore, cicd, cors, criar-prd, criar-task, criar-techspec, ddd, deploy, dev-script, dotnet-wpf, executar-task, kaizen-software, n8n-ops, pdf-generation, release, retrofit-skill, statusline, ticket, unlovable, whisper-preprocess, wsl-windows-onboarding, zitadel-idp
```
...
| **retrofit-skill** | 0.4.0 | Development | Apply non-obvious session lessons to a target skill in two modes — full (marketplace skill: bumps version, updates CHANGELOG/marketplace.json/README, commits and pushes) or lean (local skill in another repo: edits files + CHANGELOG and commits there, no bump or marketplace changes) **v0.3.0** turns "run the validator" into an actual gate: it runs **before** the commit, its WARNINGS are blocking for the skill being touched (a warning that never fails becomes background noise — 7 plugins drifted to as much as 871 chars under one), README version bumps stop being conditional (a bump *always* changes it; that "if" is how a stale 2.20.0 survived), and a snippet re-reads all four places instead of trusting that the edit landed. Also warns that `git checkout` on the shared `marketplace.json` takes your real edits with it. **(NEW v0.3.1)** o cheque de confirmação do README, que a v0.3.0 introduziu, era ele próprio um sensor cego: perguntava se `| <versão> |` existe em *algum lugar* do arquivo, e respondia `True` porque **outro** plugin estava naquela versão — medido depois de um script de edição morrer sem escrever o README. Agora extrai a versão da **linha do plugin** e imprime o valor encontrado ao lado do esperado. **(v0.3.2)** o mesmo bloco assumia um `SKILL.md` por plugin, com o nome do plugin: estourava em `FileNotFoundError` num plugin só de comandos (um cheque que não roda se lê como problema de ambiente, não como resultado) e reprovaria o estado correto num plugin multi-skill, onde cada skill tem descrição própria. Agora usa `glob` e imprime uma linha por `SKILL.md`. |
...
| **ticket** | 1.5.0 | Development | Jira ticket lifecycle for JRC Brasil projects integrated with Git — `/ticket start \| split \| close \| status`. Per-repo project detection via `.jira-project` (PROJECT/BOARD/BRANCH_PREFIX, optional BASE_BRANCH) — no hardcoded project. Prefers `acli` + atlassian MCP. **v1.0.0** initial packaging, capturing two production-proven lessons: (1) **transitions are PROJECT-SPECIFIC** — discover via `getTransitionsForJiraIssue` and transition by id (RS: `Em andamento→Aprovação→Finished`; SQ: `Em andamento→Concluído` via id 31, no Aprovação), and `acli --status` matches the DESTINATION STATUS name so `--status "Concluído"` works (the transition name "Itens concluídos" fails) → MCP transition-by-id stays a robust alternative; (2) **base branch is project-specific** (optional `BASE_BRANCH`, default = repo's detected default branch) — don't assume either `main` or `develop` — **detect** it (v1.1.1 corrected a stale claim here: SQ/sales_quote uses `develop`). **v1.0.1** corrects the acli transition guidance (the v1.0.0 corollary was inverted). **v1.1.0** fixes cards landing in the backlog without points: new issues are now **born inside the active sprint** via `acli workitem create --from-json` with `additionalAttributes` (custom fields on create need no MCP — validated on SQ), because the old create-then-edit path silently failed whenever the MCP wasn't authenticated. Adds a mandatory **read-back** after writing sprint/score (plus `sprint list-workitems` as independent proof), field-ID **discovery** via `--fields "*all"` (a plain `--json` returns 5 fields and no custom ones), fallbacks for "no active sprint" (wrong `$BOARD`, JQL `sprint in openSprints()`, kanban boards), and the measured `acli` asymmetry — `create --from-json` accepts `additionalAttributes`, `edit --from-json` rejects it, so an **existing** issue has no non-MCP path. Also documents two traps: sprint id is a **plain number** (`405`, not `{"id": 405}`), and the active sprint is the one with `state: active` **regardless of a past `endDate`**. MCP endpoint migrated from the retired HTTP+SSE (`/v1/sse`, EOL 30-Jun-2026) to Streamable HTTP (`/v1/mcp`), with `/v1/mcp/authv2` noted as the variant that answers with OAuth `resource_metadata` discovery. **v1.2.0** attacks one failure class — **the skill's own verification steps were the thing failing**, always toward the side that looks safe: `acli` prints `✗ Failure` yet **exits 0**; `--assignee` with an e-mail is refused because the session's `userEmail` isn't the Jira account (use `@me`, or accountId via REST); `sprint list-workitems` **paginates (~30)** so a fresh card falls off page 1 and reads as "not in the sprint" (replaced by JQL); and `git pull … | tail` inside an `&&` returns `tail`'s exit, so the branch is born from an unverified base (now measured with `git rev-list --left-right --count`). Two of these had been measured 17 days earlier and re-charged on SQ-107. Adds **fixVersion** end to end — REST-only (`acli` neither writes nor reads it, returning `[]` over a stored value), single-call `POST /rest/api/3/issue` carrying fixVersion + sprint + points + ADF, and the warning that Jira's `released` flag is **not** a release sensor (SQ's 0.7.1 read `unreleased` while live in production — check `origin/main`, not the metadata). Read-back is now **per field**, because the sensor differs by field. `open`/`abrir` alias `start` **v1.3.0** closes the gap the ADF guidance left open: the skill said to build ADF with a script and warned that malformed payloads are refused **without naming the node** — but the script errs too. A helper handed `"strong"` (a string) where it expected a list iterated it character by character, emitting `{"type":"s"}`, `{"type":"t"}`…; the JSON stayed syntactically valid, `json.tool` passed, and Jira answered a **silent 400**. Adds a ~10-line pre-POST sweep that walks the document and fails naming the offending marks (the only accepted ones being `strong`/`em`/`code`/`link`/`strike`/`underline`), a recipe to repair an already-built payload instead of rebuilding it, and the next suspect when the sweep comes back clean — a node `type` outside the table (`bold` for `strong`, `italic` for `em`). **v1.4.0** issue links via REST (in `POST /issueLink` the `outward` verb belongs to `inwardIssue`, so the intuitive payload writes the reverse and Jira accepts both — read back from the target issue; delete one id per call), an ADF **structure** check beside the marks check (strings inside `content` fail with the same silent 400), a "register without starting" path that skips branch and transition, and a prompt audit against the current model: personal e-mail and incident archaeology out of the body, the sprint sensor table aligned with the JQL check, caps rules in normal tone, description trimmed to 8 triggers below the cap. **v1.4.1** corrects the sensor v1.4.0 had just aligned to: right after a `POST /issue`, the JQL `key = X AND sprint in openSprints()` returns **empty** for a few seconds while the field is already stored (measured) — so following "report the failure" announces a backlog card that is in the sprint, the same false alarm `sprint list-workitems` was dropped for. The post-create verdict is now a single REST `GET` reading all five fields at once (status, assignee, fixVersions, points, sprint), JQL moves to later checks with a `key = X` tie-breaker between "not in the sprint" and "not indexed yet", and an `acli search` that matches nothing is documented as printing **nothing at all**. Adds the two ids the REST create demands and `.jira-project` doesn't hold (`project.id`, `issuetype.id`), the `{"sprints":[…]}` envelope of `board list-sprints`, and the ADF builder as a **file** rather than a piped heredoc — a typo there points at "line N of stdin". **v1.5.0** four lessons from a real `close`: `acli jira workitem comment` is a **group**, so `--key` on it answers `unknown flag` instead of "missing subcommand" (it is `comment create`); reading the comment back with `acli comment list --json` **flattens ADF to plain text**, so a perfectly stored body looks like a raw string and nearly became a reported defect that did not exist — the verdict is the REST `GET /issue/<KEY>/comment`, whose `body` is an **object** when ADF was accepted (the skill already said the exit code is not a sensor; the read-back is not one either); the RS transition table described **team convention as if it were a board constraint** — `id=31 Itens concluídos → Finished` skips `Aprovação` from `Em andamento`, and the available set **changes with the current status** (`id=6 DONE` exists only from `Aprovação`), so ids must be re-listed after each step; and `close` gained a `fixVersion` step, because `start` always asks while `close` was shipping tickets to production with no release label — stopping to ask when the version does not exist in Jira, since creating one is a project-level act. |
| **unlovable** | 1.1.0 | Development | Remove todos os traços do Lovable de um projeto gerado no lovable.dev. Separa o **cosmético** (strings de marca, README, metadados og/twitter que fazem o preview do WhatsApp "remeter ao Lovable", favicon) do **estrutural** — os dois pontos que quebram o `build` se removidos sem substituto: a feature de email (`api.lovable.dev/v1/email/send` + `LOVABLE_API_KEY`, migrada para SMTP via Nodemailer com credencial em env var) e o wrapper de config do Vite. O wrapper tem **duas formas com custo de ordem de grandeza diferente**, e a skill bifurca por elas: `lovable-tagger` é um plugin dev-only listado no próprio `vite.config.ts` (remoção local, o build não muda de forma), enquanto `@lovable.dev/vite-tanstack-config` **é** a config — removê-lo esvazia o build e exige recriar `vite.config.ts` à mão, com `server.entry` **dentro** de `tanstackStart({})`, onde fora dali o Vite ignora em silêncio. Inventário read-only antes de qualquer edição, e estrutural antes de cosmético (falha de `build` depois de dez edições de string não diz qual delas a causou). Cobre ainda o `.env` tracked sem `.gitignore` (`git rm --cached`, com a ressalva de que o segredo permanece no histórico e precisa ser **rotacionado**, não só apagado) e dois falsos positivos que custam tempo: `.tanstack/` é do TanStack e não do Lovable, e a string "lovable" no lockfile costuma ser só mirror de registry (`sandbox-npm-cache.lovable.dev`), não código. **(NEW v1.1.0)**, do primeiro uso real: a recomendação sobre `og:image` estava **invertida** — "apontar para asset próprio" soa como uma linha e não é (a tag quer URL **absoluta**, que difere entre staging e produção, e o logo do repo costuma ser faixa de cabeçalho, não o 1200x630 do card), então a saída é **remover** `og:image`/`twitter:image` com `twitter:card: summary` e registrar a arte como pendência; a verificação do §6 passa a ler o **artefato** (`grep -c lovable dist/index.html`), porque o crawler lê o build servido e não a fonte; e o falso positivo do lockfile ganha o gêmeo que não é falso — em monorepo, `npm install` da raiz não toca lockfiles órfãos de workspace, que seguem declarando o pacote como dependência real e fazem o grep acusar depois de o trabalho estar certo. |
