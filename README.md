...
| **zitadel-idp** | ✓ | ✓ | Skill — Zitadel self-hosted OIDC integration field guide (bootstrap, JWT, branding, 35 gotchas with proto-aligned v4.15 examples + CD cutover survival kit (BOOTSTRAP_ENV default, OIDC client_id numeric vs UUID, DefaultInstance feature flags, nginx-proxy VIRTUAL_PATH gotcha) + Console UI human-user creation pitfalls (Username auto-truncate, domain-policy stamp irreversible, email-verified default off) + v2.66→v4 upgrade runbook + API v1→v2 mapping) |
