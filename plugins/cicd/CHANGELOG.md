...
**Por quê:** todo o runbook até aqui partia de "o job quebra porque falta algo no runner". Numa migração real do CI de um frontend, o runner pegou o job de primeira e o `Type Check` ficou verde — e mesmo assim o CI ficou vermelho, porque o vermelho era do **repositório**. **(77)** Um CI bloqueado por cota (lição 74) não reprova nada, mas os merges continuam: apareceram 5 testes e 1 ESLint acumulados na janela cega, todos no PR que só queria trocar `runs-on`. Ficou a triagem de dois PRs (pipeline vermelho é honesto — ele revela a dívida) e a exigência de **provar** que é preexistente antes de afirmar. E uma correção de instinto que custou uma investigação inteira: dos 5 vermelhos, 3 eram **setup de teste que contradizia a própria asserção** (um deles impossível de satisfazer, porque o `beforeEach` criava o registro cuja ausência o teste exigia) — na janela cega os testes também nunca executaram, então são tão candidatos a estar errados quanto o código. O que decidiu foi um teste descartável imprimindo o estado do banco, não a leitura do código. **(78)** O `.env` é gitignored, então o CI não tem nenhum, e 9 módulos com guard de env em nível de módulo derrubaram a suíte **no import** — centenas de linhas idênticas nomeando um módulo, não um teste. **(79)** Resolvido isso, sobraram 2 snapshots: o `.snap` versionado guardava o `href` renderizado de `import.meta.env`, então aquelas variáveis têm um único valor aceitável — e `Snapshot mismatched` não diz nada sobre ambiente. **(80)** A mais sutil: `preflight` migrado para `[self-hosted, staging]` **continua passando** e vira tautológico — com o runner offline ele próprio fica `queued`, o silêncio que existia para quebrar. O critério é qual runner vigia qual: o preflight de **produção** rodando no runner de **staging** mantém o fail-fast; o **watchdog fica em `ubuntu-latest`** mesmo sob bloqueio, porque alarme morto é melhor que alarme verde que não enxerga.

## 2026-08-28 — Produção: qual runner, e quando a migração passa a valer — bump 2.22.0 → [2.23.0]

**O quê:** `self-hosted-job-migration.md` ganha **§5b** (qual runner usar quando o de produção é de organização/desconhecido; efeito só no commit taggeado; receita de `diff` para provar o job byte-idêntico) + lições 75–76 + 1 linha na Quick Troubleshooting + routing table + descrição espelhada nos 3 lugares (corrige um drift de 2 chars entre `SKILL.md` e os manifests).

**Por quê:** a v2.22.0 ensinou a migrar o `cd-staging` e ficou verde. Ao aplicar o mesmo no `cd-production.yml` de um repo real, a skill não respondia às duas perguntas que a tarefa exigiu. **(75)** O runner de produção era de **organização** — `gh api repos/…/actions/runners` só mostra os de repositório e `orgs/…/actions/runners` devolve `403` sem `admin:org` — logo não havia como saber se tinha Docker para o `services:`, buildx para o push ou yarn. O reflexo de espelhar `[self-hosted, production]` no `ci` estrearia um runner desconhecido **no dia da tag**. A resposta foi rodar `ci` e `build-and-push` no runner de **staging**, que já tinha executado exatamente esses jobs (run verde no mesmo dia), e deixar só o `deploy` no de produção — a imagem vai ao GHCR, produção só puxa. O job `ci` foi copiado e provado byte-idêntico com `diff` sem comentários. **(76)** `cd-production` dispara por tag, e o GitHub executa o arquivo **do commit taggeado**: a migração tinha de estar mesclada antes da tag, e não existe dry-run — a evidência ficou sendo o `cd-staging` verde com jobs idênticos mais o CI do PR (que também rodou no self-hosted). Observação de distribuição: a sessão que fez isso carregou a **v2.20.0 do cache** do plugin apesar de a 2.22.0 já estar instalada — a routing table nova não foi vista; vale um `/plugin update` depois de cada bump.

## 2026-08-27 — Migrar job para self-hosted: as armadilhas que a alavanca de custo não contava — bump 2.21.1 → [2.22.0]

**O quê:** nova reference `references/self-hosted-job-migration.md` (pré-voo por ordem de risco medida, 4 armadilhas com assinatura de log, e o padrão de descobrir a topologia em vez de supor) + `ci-cost-minutes.md` §5 nova (bloqueio de cota/cobrança), §3a re-rankeada e **§3b corrigida** + lições 71–74 + lição 68 emendada + 2 linhas na Quick Troubleshooting + linha na routing table + 6 keywords + descrição enxugada (473 → 467 chars). Corrige também um **drift de versão**: o `SKILL.md` marcava `2.20.0` enquanto `plugin.json`/`marketplace.json` estavam em `2.21.1`.

**Por quê:** a v2.21.0 ensinou a *puxar* a alavanca (§3a: "mova `ci`/`build-and-push` para o self-hosted ocioso") mas não o que acontece depois. Seguindo esse próprio conselho num repo real, o job levou **cinco execuções** para ficar verde — e a lista "Verifique antes" **rankeava o risco errado**: `service containers exigem Docker no runner`, dado como "o ponto de falha mais provável", passou de primeira (`Initialize containers` verde já na 1ª tentativa). Quebraram três coisas que a skill não nomeava.

**(71)** `actions/setup-node` com `cache: 'yarn'` **invoca o binário do yarn** para montar a chave do cache — `ubuntu-latest` traz yarn pré-instalado, self-hosted não, e o step morre com `Unable to locate executable file: yarn` **antes** de qualquer `yarn install`, logo antes de qualquer chance de instalá-lo. **(72)** `P1001` contra um service container **`healthy`** — o healthcheck roda DENTRO dele e não prova alcance; se o runner é conteinerizado, `-p 5432:5432` publica no host e `127.0.0.1` no job é o loopback do runner. **(73)** `container:`, a correção aparentemente óbvia de (72), **não sobe em runner desatualizado**: o GitHub monta os `externals` em `/__e` e força Node 24, e um runner antigo devolve `exec: "/__e/node24/bin/node": no such file` — o `checkout` nem executa.

**Método, registrado em §6 da nova reference:** foram **quatro rodadas gastas em hipóteses lidas do YAML** contra **uma** que respondeu — a que fez o job imprimir `hostname`, `/.dockerenv` e a tabela de rotas. Sem SSH para o host do runner, **o job é o único instrumento de medição**, e isso deve vir na primeira falha, não depois do terceiro palpite. Duas hipóteses ficaram registradas como refutadas para não voltarem: o Docker publicou em `0.0.0.0` **e** `[::]:5432` (mata a teoria do IPv6), e a premissa de (73) — runner em container — estava **certa**, só a correção não passava. Junto veio uma armadilha de sonda: `ss -ltn 2>/dev/null` imprimiu nada, o que se lê como "nenhuma porta escutando", mas era `command not found` engolido pelo redirect.

**(74)** E o modo de falha que não se parece com cobrança: três jobs "falharam" em ~3s, com **zero steps**, `runner_name` vazio e `log not found`. Parece build quebrado; é a conta bloqueada. A mensagem existe **só nas annotations do check-run**, que nenhum comando de log alcança. Isso muda a prioridade da §3a: **self-hosted não passa pela cota**, então mover o gate de CI para lá deixa de ser economia e vira o que faz o gate **existir**.

**Correção de conteúdo errado (§3b):** a reference recomendava `paths-ignore` em `pull_request` explicando que "só pula quando todos os arquivos do **push** casam". Em `pull_request` o GitHub avalia o diff **INTEIRO** do PR (`base...head`) — medido: um commit tocando só `.claude/**` e `docs/**` disparou o run completo num PR de 148 arquivos. A alavanca paga no `push`; em PR ela pula apenas um PR 100% documentação.
