...
**O quê:** nova reference `references/self-hosted-job-migration.md` (pré-voo por ordem de risco medida, as 4 armadilhas com assinatura de log, e o padrão de descobrir a topologia em vez de supor) + nova seção `ci-cost-minutes.md` §5 (bloqueio de cota/cobrança) + §3a re-rankeada + **§3b corrigida** + lições 71–74 + lição 68 emendada + 2 linhas na Quick Troubleshooting + linha na routing table + 4 keywords + descrição enxugada (473 → 467 chars). Corrige também um **drift de versão**: o `SKILL.md` marcava `2.20.0` enquanto `plugin.json`/`marketplace.json` estavam em `2.21.1`.

**Por quê:** a v2.21.0 ensinou a *puxar* a alavanca (§3a: "mova `ci`/`build-and-push` para o self-hosted ocioso") mas não o que acontece depois. Ao seguir esse próprio conselho num repo real, o job levou **cinco execuções** para ficar verde — e a lista "Verifique antes" da skill **rankeava o risco errado**: `service containers exigem Docker no runner` era dado como "o ponto de falha mais provável" e passou de primeira (`Initialize containers` verde na primeira tentativa, healthcheck OK). O que quebrou foram três coisas que a skill não nomeava.

**(71)** `actions/setup-node` com `cache: 'yarn'` **invoca o binário do yarn** para montar a chave do cache. `ubuntu-latest` traz yarn pré-instalado; self-hosted não — e o step morre com `Unable to locate executable file: yarn` **antes** de qualquer `yarn install`, ou seja antes de qualquer chance de instalá-lo. **(72)** `P1001: Can't reach database server at localhost:5432` com o service container **`healthy`** — o healthcheck roda DENTRO do container e não prova alcance. Se o runner é conteinerizado, `-p 5432:5432` publica no host e `127.0.0.1` no job é o loopback do runner. **(73)** `container:`, a correção aparentemente óbvia de (72), **não sobe em runner desatualizado**: o GitHub monta os `externals` em `/__e` e força Node 24, e um runner antigo devolve `exec: "/__e/node24/bin/node": no such file` — o `checkout` nem executa.

**A lição de método que ficou registrada em §6 da nova reference:** foram **quatro rodadas gastas em hipóteses lidas do YAML** (IPv6; `container:`; ordem dos candidatos) contra **uma** que respondeu — a que fez o job imprimir `hostname`, `/.dockerenv` e a tabela de rotas. Sem SSH para o host do runner, **o job é o único instrumento de medição**, e isso deve vir na primeira falha, não depois do terceiro palpite. Duas hipóteses foram refutadas por medição e estão registradas como tal para não voltarem: o Docker publicou em `0.0.0.0` **e** `[::]:5432` (mata a teoria do IPv6), e a premissa de (73) — runner em container — estava **certa**, só a correção é que não passava. Armadilha de sonda que veio junto: `ss -ltn 2>/dev/null` imprimiu nada e se lê como "nenhuma porta escutando", mas era `command not found` engolido pelo redirect.

**(74) E o modo de falha que não se parece com cobrança:** três jobs "falharam" em ~3s, com **zero steps**, `runner_name` vazio e `log not found`. Parece build quebrado; é a conta bloqueada. A mensagem existe **só nas annotations do check-run** (`gh api /repos/<o>/<r>/check-runs/<job_id>/annotations`), que nenhum comando de log alcança. Isso muda a prioridade da §3a: **self-hosted não passa pela cota**, então mover o gate de CI para lá deixa de ser economia e vira o que faz o gate **existir** — um `ci.yml` hospedado sob bloqueio não reprova nada, só não roda, e o PR fica sem rede de segurança sem ninguém notar.

**Correção de conteúdo errado (§3b):** a reference recomendava `paths-ignore` em `pull_request` como alavanca e explicava que "só pula quando **todos** os arquivos do **push** casam". Em `pull_request` o GitHub avalia o diff **INTEIRO** do PR (`base...head`), não o push — medido: um commit tocando só `.claude/**` e `docs/**` disparou o run completo num PR de 148 arquivos. A alavanca só paga no `push`; em PR ela pula apenas um PR 100% documentação.
