...
| 68 | `[S]` | Custo do Actions é por **JOB arredondado para cima ao minuto** — a contagem de jobs é alavanca independente da duração, e o self-hosted é capacidade grátis já paga | Repo **privado** consome cota (público não). Um job de 51s custa **1 minuto cheio**, então o instinto de paralelizar (`lint` e `test` separados) **ADICIONA** um minuto por execução — compra wall-clock com fatura. E o maior desperdício costuma ser estrutural: `build-and-push` e o gate de CI queimando minutos em `ubuntu-latest` enquanto o runner self-hosted, que a org já paga, roda só o `deploy` por 47–103s/dia. Nada nesses jobs exige runner hospedado. Gotchas da migração: **service containers exigem Docker no runner**; o build passa a usar CPU/disco do host de deploy (mas o cache de camadas fica quente); **um runner roda um job por vez** (jobs antes paralelos serializam); replicar o step de limpeza de workspace. Rollout: staging → observar um deploy real → produção. Ver `ci-cost-minutes.md` §1/§3a |
