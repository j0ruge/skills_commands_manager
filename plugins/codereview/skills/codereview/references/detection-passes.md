...
**Why this pass exists**: the typical shape is a PR that grows an exported tuple (say from 4 members to 7, with a docstring explaining why) without touching the contracts test that still asserts the old 4. The test goes red on every branch; each contributor reproduces "fails on baseline too" with `git stash`, dismisses it, and nobody root-causes it. Static review at the originating PR catches it deterministically — the diff modified the const, the test asserts the old shape, and a grep across the codebase makes the mismatch visible. This is the same family as 6.5.2 (project docs drift when code changes) generalized to test artifacts that assert literal contracts.
...
- **Over-export — run this check on EVERY "unused export" finding before flagging.** `knip`/`ts-prune` count only *cross-file* references, so they label any export with no external importer "unused" even when it's alive. Before recommending anything, grep the symbol both (1) inside its defining file and (2) in the signatures of *exported* declarations. There are two sub-cases with **opposite** fixes — applying the wrong one breaks live code:
  - **(a) In-file plumbing** — used only inside its own file, not part of any exported signature → recommend **dropping the `export` keyword** (make it module-private), **not** deleting it. Flag at **LOW**: *"used only in-file — remove `export`, keep the symbol."* Motivating case: a `DefField` helper used 11× in its own module, reported as an unused export — deleting it would have broken the page.
  - **(b) Public type-surface / intentional API** — the symbol appears in the signature of an *exported* symbol (e.g. an interface typing a field or param of an exported type or hook return), or is a deliberately-published helper → **keep the `export`**. Dropping it here is wrong: it breaks the public type contract and can trigger the TS error *"exported X has or is using private name Y"* (TS4023/TS4094) under declaration emit / composite `tsc -b` builds. If the tool still flags it, recommend **marking it intentional** — a `@public` JSDoc tag (knip honors it) or `@internal` — never narrow or delete. Flag at **LOW**: *"part of exported surface — mark `@public`, keep exported."* Motivating case: arg-types like `UpdateArgs` inside an exported `UseCotacaoMutationsReturn`, and `AuthUser` typing the `user` field of an exported `UseAuthReturn` — flagged as unused exports though they're live contract.
