...
These are hard-won insights from iterating on a 2h13min lecture recording and a multilingual (PT+JA) meeting; each one names the failure it prevents:

### 1. No `afftdn` (FFT noise reduction) on the transcription path
...
Enhanced WAV files are enormous (2.6 GB for 2h). The listenable `*_enhanced.opus` is encoded at
**48 kHz, 64 kbps, `-application audio`** (wideband, fuller voice; `-application voip` is
narrowband telephony tuning and thins the voice). Bitrate/sample-rate are configurable via
`--listen-bitrate` / `--listen-sr`.

### 9. The listening copy is built from the original file, with stable gain only (anti-"picotamento")
`*_enhanced.opus` is the file a human listens to. It is generated by `build_listenable()` from the
**original** input, **decoupled** from the transcription path, because that path stacks three
gain-modulation/chopping sources — `silenceremove` (cuts quiet speech), `acompressor release=50ms`
(pumps every syllable gap) and single-pass `loudnorm` (a dynamic AGC that "breathes" on pauses) —
and together they make a low-volume or impaired (dysarthric, slow) speaker sound choppy, cutting
between syllables ("picotando").

The listening copy therefore uses:
- wideband (48 kHz) and **no silence removal** by default → continuous audio, the listener follows
  every pause and nothing is clipped (`--trim-listen` enables a gentle cut when the user wants it);
- **stable gain only** — slow-release compressor (`release=400ms`) + fixed makeup + true-peak
  `alimiter` with lookahead, and no dynamic AGC. A 2-pass `loudnorm linear=true` (the textbook
  anti-pumping move) falls back to dynamic normalization when the source clips — measured on a
  real recording — so it is not used here either;
- a limiter ceiling with headroom (`--listen-limit 0.6`): Opus encoding adds inter-sample overshoot
  above the ceiling, and without headroom the decoded true-peak goes above 0 dBFS.

The **transcription path** runs at 16 kHz with `silenceremove` at -30dB, then the enhancement
chain, with `detection=rms`, `window=0.025`, `stop_silence=0.5`, `stop_duration=2.0` — gentle
enough to keep slow speakers intact for Whisper. Lessons #1 (no `afftdn`) and #3 (-30dB) apply to
it; denoise on the listening copy is **opt-in and off by default** (`--denoise`).

### 7. UTF-8 everywhere on Windows (handled automatically)
On Windows the default console encoding is cp1252. When `whisper` CLI prints transcription progress containing accented or CJK characters it crashes with `UnicodeEncodeError`, and the Python `subprocess.run` reader thread separately crashes with `UnicodeDecodeError` on bytes > 0x7F. Without protection, all segments come back empty with only a "Segmento vazio" warning. The pipeline sets `PYTHONIOENCODING=utf-8`, reconfigures `sys.stdout/stderr`, and forces `encoding="utf-8", errors="replace"` on every `subprocess.run` — no user action needed. If you ever invoke `whisper` standalone on Windows, set `$env:PYTHONIOENCODING="utf-8"` first.
...
Copy `scripts/whisper_preprocess.py` **and** `scripts/merge_passes.py` to the user's working directory — the pipeline imports `merge_passes` from its own directory at merge time; with only the first file, `--secondary-language` still produces both passes but logs `Falha ao mesclar passes` and no `*_transcricao_merged.txt` appears. Then:
...
The merge logic lives in `scripts/merge_passes.py` (imported by the pipeline) and can be called standalone:
```bash
...
| Voice choppy / cutting between syllables ("picotando") in `*_enhanced.opus` | Silence removal, a fast compressor or a dynamic AGC applied to the listening copy (lesson #9) | Keep `--trim-listen` off and the stable-gain chain as is; for a very quiet speaker raise `--listen-makeup` (e.g. 7) instead of adding gain stages |
...
| Huge intermediate files | WAV format | Intermediates are removed at the end unless `--keep-intermediate` is passed |
| "Segmento vazio" with no real error | Windows cp1252 console encoding (lesson #7) | The script forces UTF-8 and logs the tail of whisper's stderr when exit≠0 or output is empty — read that stderr |
