...
- **Quirk 41 — Idempotent bootstrap creates initial user grants but does NOT reconcile existing ones when the YAML evolves.** When a new app + role is added to `seedUser.roles` (e.g., a second SPA `quote.admin` alongside existing `battery.admin`), the bootstrap detects the existing grant as `ALREADY_EXISTS` and moves on; the grant's `roleKeys` array stays at its day-0 set indefinitely. JWTs for the seed user keep going out missing the new role, the new SPA's `parseClaims`/`requireRole` returns 401 (`role_nao_reconhecida`). **The diagnostic asymmetry** that makes this hard is: a *brand-new* user provisioned via the same bootstrap works fine immediately, so "the bootstrap works" — but the operator most likely to test first (the pre-existing seed user) is exactly the one that breaks. Easily confused with quirks 11 (access vs id token claims), 12/13/36 (the "401-storm with apparently-valid JWT" family — same backend-facing symptom), or 28 (Login UI v2 blocking). The diagnostic step is to decode the JWT and inspect `urn:zitadel:iam:org:project:roles` — if the new role's key is absent, this is the grant gap, not any of the others. Fix imediato via Quirk 8 pattern (search-then-PUT em `/management/v1/users/{u}/grants/{g}`); fix durável é estender o bootstrap pra reconciliar grants existentes quando o `roleKeys` desejado é strict superset do atual.

- **Quirk 42 — Browser SPA → backend Express needs CORS; preflight `OPTIONS` hits `authJwt` and 401s, mimicking quirks 12/13/36.** Stack típico OIDC SPA pareia `localhost:5173` (Vite) com `localhost:3000` (Express). Isso é cross-origin (portas diferentes), e todo request com `Authorization` dispara preflight `OPTIONS` que o browser NÃO pula. Sem CORS middleware (ou com CORS depois do auth), preflight cai no `authJwt` → sem Bearer (preflight não carrega por spec) → 401 `jwt_ausente`. Browser vê 401 + `Access-Control-Allow-Origin` faltando, aborta o request principal, SPA observa "todas as chamadas falham 401" — sintoma idêntico ao 401-storm family. **Assimetria diagnóstica chave**: mesmo backend probado com `curl -H "Authorization: Bearer <JWT>"` retorna 200 (sem Origin → sem preflight); só o browser falha. Fácil de misdiagnosticar como JWT signature/audience/issuer mismatch, especialmente porque integration tests (MSW ou supertest) passam verdes — nenhum dos dois emite preflight CORS real, então o bug é invisível até um browser real estar no loop. Cura é CORS middleware como PRIMEIRO middleware app-level, short-circuit `OPTIONS` com 204 + headers. Prevenção é UM spec Playwright que autentica contra Zitadel real e bate em um `/api/*` endpoint — pega isso no primeiro commit.

- **`spa-recipes.md` — new Recipe "E2E browser tests (Playwright) against self-signed Zitadel"** documenta `ignoreHTTPSErrors: true` no `use` do `playwright.config.ts` (counterpart browser-side do Quirk 12 `NODE_EXTRA_CA_CERTS` no backend — sem isso o fetch da discovery do `oidc-client-ts` falha silently com `ERR_CERT_AUTHORITY_INVALID` e o `react-oidc-context` engole a rejeição via `void`; sintoma é "clicar Entrar não faz nada", mesma família do Quirk 16). Também cobre dois gotchas adjacentes que vale pré-cozinhar no spec: (1) Zitadel Login UI v1 pré-preenche o username quando `login_hint` é mandado pelo `oidc-client-ts`, deixando o input `loginName` `class="hidden"` — specs que sempre tentam preencher esse input timeout em visibilidade, fill condicional é correto; (2) `storageState` reuse do Playwright é não-determinístico com `InMemoryWebStorage` (cookie sobrevive mas o User object não, boot-time `signinSilent` por iframe não estabiliza dentro do watchdog de 5s no Chromium headless, `<SessionResolving>` trava) — re-login por teste é o default pragmático pra suites pequenas.

- **Quirk 30 — extended com bullet sobre staleness do `bootstrap.json`**: depois de editar o YAML pra adicionar um `applications[]` novo, consumer downstream que lê o `bootstrap.json` pra extrair `clientId` vê o snapshot do boot *anterior* até o bootstrap rodar de novo. Sempre re-rodar antes de extrair IDs. Mesma família de higiene dev/prod do quirk original.

- **Quirk 39 — extended com nota operacional** em `ZITADEL_SEED_USER_PASSWORD`: o bootstrap lê o env com guard fail-fast (`ausente ou curta (<12)`). Dev workflows que wrapam o bootstrap com `dev.sh` tipicamente geram a senha uma vez e persistem em `.env` na raiz do repo, então usuários de `dev.sh` nunca veem essa armadilha. CI re-runs e re-execuções humanas de `docker compose --profile bootstrap run` direto precisam exportar o env explicitamente — olhar no `.env` primeiro se um `dev.sh` já provisionou; senão gerar fresh com o padrão `Aa1!`-prefix do corpo do quirk. Fecha o footgun específico de "rodei o mesmo comando de bootstrap dos docs e crashou antes de fazer qualquer coisa."

### Adicionado

- `SKILL.md` — "forty" → "forty-two" quirks; entries 41 (seed grant reconciliation) e 42 (browser CORS preflight 401) novas em formato consistente com 36-40; description em frontmatter atualizada para `42 documented quirks` com triggers novos (`user grant not applied`, `CORS preflight 401`, `Playwright self-signed Zitadel`). Quirks 30 e 39 estendidos inline com as notas operacionais menores acima.
- `references/troubleshooting.md` — novas entries sob "JWT validation errors": `### Seed user JWT missing role added later to YAML` (Quirk 41) e `### Browser 401 storm on cross-origin API, same backend OK via curl` (Quirk 42). Ambas no formato canônico Symptom/Cause/Fix com snippets de diagnose e cura.
- `references/spa-recipes.md` — nova seção `## Recipe — E2E browser tests (Playwright) against self-signed Zitadel` ao final, cobrindo config do `ignoreHTTPSErrors`, fill condicional do username pra fluxos com `login_hint`, e o caveat do `storageState`+`InMemoryWebStorage` reuse.

### Por que minor (não patch)

2 quirks novos + 1 recipe nova (~80 linhas) + 2 quirk-updates inline + 2 troubleshooting entries é conteúdo substantivo equivalente em magnitude aos bumps anteriores (v0.5.0 / v0.6.0 / v0.7.0 / v0.8.0 cada um adicionou 3 quirks). Pre-1.0 minor é o padrão pra essa magnitude. Nenhuma quebra de backwards-compat — quirks/recipes só adicionam guia operacional.

---

## 2026-05-08 — Smoke-e2e CI quirks: `admin.pat` bind mount EACCES cascade, default password policy 4-class, Login UI v2 healthcheck slow on small runners — bump 0.7.0 → 0.8.0

Source: project `validade_bateria_estoque`, PR #10 (mergeada 2026-05-08, merge commit `13eed8d`). Destrava do `smoke-e2e` no `idp-ci` workflow expôs uma cebola de 4-5 camadas de quebra que tinham apodrecido por meses sob `continue-on-error: true` — cada fix descobriu o próximo. As três que viraram quirks novos vivem todas no contexto CI (não dão na cara em dev por motivos diferentes — perms do user, scaffolding do `dev.sh`, generosidade de timeout local).

- **Quirk 38** — `ZITADEL_FIRSTINSTANCE_PATPATH=/current-dir/admin.pat` escreve em path dentro do bind mount `./zitadel/local:/current-dir:rw`. O processo Zitadel roda como uid 1000 (default da imagem); no GHA runner `ubuntu-latest` a pasta nasce post-checkout owned por `runner:docker (uid 1001)` com mode `0755` — uid 1000 não escreve. **A pegadinha venenosa é o cascateamento**: o primeiro boot falha em `03_default_instance` com `permission denied`, mas Zitadel `setup failed, skipping cleanup` deixa estado parcial no eventstore (a row `instance_domain=127.0.0.1.sslip.io`). Toda retry sob `restart: always` falha agora com `unique_constraints_pkey` na MESMA migration — o operador olha o bottom dos logs, vê o duplicate key, abre o Quirk 28 (#8910/#9293), aplica a cura errada (remover `LOGINCLIENT_*` envs), nada muda. Diagnóstico canônico: **sempre subir os logs até a PRIMEIRA tentativa de migration**; se ela falhou com `permission denied`, é Quirk 38 (cura `mkdir -p && chmod 0777`), não Quirk 28. Em dev a pasta nasce com perms do user via `dev.sh` ou similar, então o bug é CI-only.

- **Quirk 39** — Política default de senha do Zitadel exige TODAS as 4 char classes (upper/lower/digit/symbol). `openssl rand -hex 16` é só `[0-9a-f]` e morre deterministicamente em `AddHumanUser` com `400 invalid_argument: Password must contain upper case (COMMA-VoaRj)` no step de seed user. "Fix" só com prefixo upper e você bate na próxima classe ausente (digit ou symbol). Solução: gerador estruturado `Aa1!${RAND_TAIL}` (tail alfanum 28 chars do `tr -dc 'A-Za-z0-9'` + `/dev/urandom`) — 32 chars total, todas as 4 classes garantidas, alfanum no random evita gotchas de base64 padding/slash. Mesmo padrão funciona contra qualquer policy-shaped service.

- **Quirk 40** — `zitadel-login` (container Next.js da Login UI v2) precisa ~90s+ pro primeiro render do healthcheck `wget --spider http://localhost:3000/` em runner `ubuntu-latest` shared (2 vCPU). Com `start_period: 30s` + `interval: 10s × retries: 12` no compose e `--wait-timeout 120` no comando, a janela de wait morre antes do primeiro render. **`zitadel`, `zitadel-init`, `zitadel-db` ficaram Healthy bem antes** — o wait que estoura é puramente da Login UI, e bootstrap + REST integration tests **não exercitam a Login UI** (bootstrap lê `login-client.pat` do volume nomeado escrito pelo `zitadel`; tests REST batem em `zitadel:8080` direto). Solução: scope `docker compose up -d --wait --wait-timeout 120 zitadel-db zitadel-init zitadel` — compose só espera os serviços listados; `zitadel-login` e `mailpit` simplesmente não sobem. Em dev o `up` default mantém tudo. Se o test específico precisar Login UI healthy (Playwright login spec), bumpar `--wait-timeout` para ~240s. **Sempre** incluir `zitadel-login` no on-failure log dump (`docker compose logs zitadel-login || true`) mesmo quando não está no `--wait` — sem isso, debugar a próxima vez é cego.

Além dos 3 quirks, este bump adiciona uma **nova seção §"Smoke-e2e plumbing checklist for GHA"** em `docker-compose-bootstrap.md` que enumera as 6 camadas em ORDEM de quem mascara quem: (1) chmod 0777 do bind mount, (2) scope `--wait`, (3) password 4-class, (4) `prisma generate` + `prisma migrate deploy` no Postgres efêmero (vitest collect dies com `ERR_MODULE_NOT_FOUND` sem o generated client; SELECTs morrem com `relation does not exist` sem migrate), (5) capturar `projectId` de `bootstrap.json` para `$GITHUB_ENV` (sem isso, integration tests com `envReady()` guards skipam silenciosamente porque faltam `ZITADEL_PROJECT_ID`/`OIDC_AUDIENCE`/`ZITADEL_PAT_FILE`), (6) sempre dumpar logs no on-failure step. **A ordem importa**: tentar consertar (4) antes de (1) é trabalho perdido — a falha mais cedo mascara toda camada posterior. E o meta-aviso: `continue-on-error: true` em smoke-e2e que apodrece por meses é uma armadilha; ou se compromete a manter verde com tracking quando vermelho, ou aceita que é puramente informativo e move tests que importam pro `build` blocking.

A descoberta também surfaceou um bug ainda em diagnóstico (`CreateAuthorization` v2 falha com `Project Grant not found COMMAND-4m9ff` no handler `RegisterOperator → assignProjectRole` mesmo o mesmo endpoint funcionando no bootstrap) que NÃO virou quirk — sem root cause, vira ruído. Está documentado no seed PRD `specs/_backlog/authorization-v2-project-grant-not-found.md` do projeto-fonte; promove a quirk quando alguém diagnosticar.

### Adicionado

- `references/troubleshooting.md` — nova seção `## CI / smoke-e2e errors (GHA runner)` com 3 entries cobrindo Quirks 38/39/40, cada um com Symptom/Cause/Fix no formato canônico.
- `references/docker-compose-bootstrap.md` — nova seção `§"Smoke-e2e plumbing checklist for GHA"` com checklist de 6 passos em ordem causal.
- `SKILL.md` — bump "thirty-seven quirks" → "forty quirks" no headline list; entries 38/39/40 adicionadas em formato consistente com 36/37; description em frontmatter atualizada para `40 documented quirks` (era `27 documented quirks`, drift histórico) com triggers novos `smoke-e2e CI`, `admin.pat EACCES`.

### Por que minor (não patch)

3 quirks novos + 1 nova seção de referência (`Smoke-e2e plumbing checklist`) é conteúdo substantivo, não fix isolado. Pre-1.0 minor é apropriado para essa magnitude — segue o padrão de v0.5.0/v0.6.0/v0.7.0 que também adicionaram 3 quirks cada.

---

## 2026-05-08 — Production-cutover quirks: backend JWKS reachability + frontend defense-in-depth against 401-storm-revokes-session — bump 0.6.0 → 0.7.0

Source: project `validade_bateria_estoque`, hotfix PR #9 (mergeada 2026-05-08, merge commit `d3b2aab`). Production was 401-storming after the v0.6.0 cutover even though the bootstrap, JWT validation and SPA wiring all looked correct. Two distinct bugs hidden by the same symptom (`401 with apparently-valid JWT`) — one network-level on the backend, one race-condition cluster on the frontend.

- **Quirk 36** — Backend container that validates JWT needs `extra_hosts: idp.<domain>:host-gateway` when the IdP is co-located on the same VPS host with unreliable hairpin NAT. `jose.createRemoteJWKSet` caches the JWKS for `cacheMaxAge: 600s` default — within that window every JWT verification reuses the cached keys and works fine. After the TTL, the next `jwtVerify()` triggers a `reload()` that does an ordinary Node `fetch` on the JWKS URL. In single-host self-hosted deployments where the backend container resolves `idp.<domain>` via public DNS to the host's external IP, hairpin NAT (DNS → external IP → loopback to local nginx-proxy) is flaky on many VPS providers — the reload's TCP handshake silently fails, jose raises `JOSEError`, the validator throws `InvalidTokenError`, and **every authenticated request 401-storms** even though the JWT is decoded-perfect (`iss`/`aud`/`exp`/`kid` all verifiable, the `kid` is verifiably present in the JWKS endpoint when fetched from outside the container). The pathognomonic symptom is **"backend works for ~10 minutes after restart, then 401-storms forever"** — uptime correlates with the JWKS TTL window. Family of quirks 12 (self-signed JWKS) and 13 (volume reset stale clientId) — same symptom, different layers; this is the third documented cause of "401-storm with apparently-valid JWT" in this skill. **Fix**: `extra_hosts: ["idp.<domain>:host-gateway"]` on the backend service in compose, mapping the IdP hostname to the docker-bridge IP — same path external traffic takes, no hairpin NAT involvement. Apply to ANY co-located container that talks to the IdP (bootstrap, observability sidecars, runners). Captured in **`token-validation.md` "Network reachability to the IdP from the JWT validator"** with diagnose-in-3-commands ladder, fix template, and a logging tip (use `pino.warn` not `console.error` for `JOSEError` — same path also fires for malformed tokens from clients/attackers, so logging at `error` level is a log-spam vector). Also surfaced in `troubleshooting.md` "401 storm starting ~10 min after backend restart" with the disambiguator vs quirks 12/13.

- **Quirk 37** — Frontend defense-in-depth against the 401 → silent-renew → RT-reuse → session-revoke cascade requires THREE coordinated layers, not just dedupe at the provider level. Zitadel has refresh-token rotation with reuse detection enabled by default; the moment two requests use the same RT, Zitadel **revokes the entire session** — every AT and RT becomes invalid. Re-login creates a fresh session, but if the underlying root cause persists (or if the provider's dedupe leaks via the `addAccessTokenExpiring` handler that calls `signinSilent` directly), a new storm revokes that too → infinite loop. **Three independent races** feed the cascade: (1) concurrent 401-retries from N parallel queries; (2) the proactive `addAccessTokenExpiring` event vs an in-flight 401-retry — two callers, same RT; (3) TanStack Query default `retry: 3` amplifies any 401 by 3× requests/refreshes per query. **The three layers that close it**: (L1) dedupe lives in `ApiClient` (instance field `pendingRenew`, cleared in `.finally()`) + expose `apiClient.refreshToken()` public — both the internal 401-retry AND the external `addAccessTokenExpiring` handler go through the same Promise; provider-level `useRef` dedupe alone leaves L2 race unblocked because the expiring handler typically calls `signinSilent` directly. (L2) TanStack `retry` predicate filters `ApiError 401` — 401 is not transient, retrying amplifies the storm. (L3) listener for `apiclient:unauthorized` CustomEvent in `<AuthProvider>` with `isAuthRoute()` early-return guard + `state: { returnTo: location.pathname+search+hash }` on `signinRedirect` — without the guard, a 401 emitted by `/auth/callback` triggers another `signinRedirect` → loop; without `state.returnTo` the user lands on `/` after re-auth instead of where they were. The three together make the SPA recover gracefully whether the cause is transient (network blip), persistent (IdP misconfiguration like quirk 36), or downstream (session revoked by another tab). Captured as **new "Recipe — Defense in depth against 401-storm-revokes-session"** in `spa-recipes.md` — code samples for all three layers, "what each layer alone leaves leaking" table, validation via Playwright spec that drives 3 concurrent fetches against an endpoint that 401s once, and explicit note on TanStack Query `failureCount` semantics (predicate sees the count BEFORE the increment, so `failureCount < 1` ≡ `retry: 1` shorthand, NOT zero retries — easy to get wrong by inspection alone).

Both lessons stem from the same hotfix: the 5-fix PR landed extra_hosts (root cause) + dedupe-at-ApiClient + listener-with-guard + retry-filter + pino-logger. Code review (Copilot) caught the dedupe-leak via expiring handler during the PR (originally the dedupe was at provider level), pushing the fix into shape L1 + L3 see now. The 5th fix in the PR (substituting `console.error` for `pino.warn` in the validator) reinforces the "warn not error" guidance in `token-validation.md`.

This bump also fixes a long-standing description drift in `plugin.json` ("28 documented quirks" lead while body referenced quirks up to 35) — now reads "37 documented quirks" consistently.

---

## 2026-05-07 — Console UI human-user creation pitfalls: 3 quirks for the manual operator flow — bump 0.5.0 → 0.6.0

Source: same project (`validade_bateria_estoque`), first real human users provisioned in production via Console after the feature 006 cutover. The skill so far covered API/bootstrap-driven creation thoroughly, but the moment TI started creating users manually in the Console UI, three brand-new failure modes appeared — none of which trigger via API and none documented before. Captured a complete operator-facing KB (`docs/kb/criar-usuario-logistica.md`) and the lessons feed back here.

- **Quirk 33** — Console v4 "Add Human User" form auto-truncates `Username` to the email's local-part on auto-fill. Operator types `tadeu.mezzavilla@jrcbrasil.com` in **E-Mail**, the form quietly populates **Username** with `tadeu.mezzavilla`. Combined with `userLoginMustBeDomain=false` (instance default), the canonical `loginName` becomes just `tadeu.mezzavilla` — and the end user, naturally, types their full email at the login screen and gets **"O usuário não pôde ser encontrado" / "User could not be found"**. The user exists, has the right grant, email is correct in the contact section. Cost ~45 minutes of "the user is there, why can't they login" plus a Playwright diagnostic in production to spot the truncated `Preferred Login Name` column. **API creation does NOT trigger this** because `AddHumanUserRequest` honors whatever `username` you pass — only the manual UI flow has the auto-truncate. Operationally this will be the #1 "user can't login after creation" cause in any self-hosted Zitadel where humans are provisioned manually.

- **Quirk 34** — `userLoginMustBeDomain=true` (Settings → Domain settings → "Add Organization Domain as suffix to loginnames") **stamps loginNames irreversibly on existing users**. Toggling the org-level flag rewrites every existing user's loginName to `userName + "@" + orgPrimaryDomain`. **Disabling the flag and clicking "Reset to Instance default" does NOT undo the stamp** — affected users keep the suffix indefinitely until you edit their userName to a temp value + Save + change back to force a recompute. Doubly hostile in self-hosted: the org `primaryDomain` auto-generates as `<orgSlug>.<externalDomain>` (e.g., `jrc.idp.jrcbrasil.com`), unrelated to the company's email domain (`jrcbrasil.com`), so toggling the flag without first adding a custom domain via Organization Domains produces nonsensical loginNames or, worse, double-suffixed `user@jrcbrasil.com@jrc.idp.jrcbrasil.com` if userName already had `@`. Discovered live in production while attempting Quirk 33's "structural fix" — turning on the suffix policy was supposed to make `userName` always be `username@domain`, but it stamped an additional `@<auto-generated-domain>` and broke the user further. Recovery via per-user userName twiddle was painful enough to warrant a delete+recreate. Skill now flags this clearly and recommends NEVER enabling the toggle in production until a custom domain is verified at the org.

- **Quirk 35** — Console "Add Human User" leaves "E-mail Verificado" / "Email verified" **unchecked by default**, and the operator easily misses it. The user is created fine, login advances past username + password, then stalls on a "Verifique seu e-mail / Code sent" screen waiting for an SMTP code that — in self-hosted deployments where SMTP is commonly deferred — never arrives. **API creation avoids this** because `AddHumanUserRequest` lets you set `email.isVerified: true` explicitly in the payload. Cost ~15 minutes of "wait, we're not even on SMTP yet, how is it sending an email?" before realizing the checkbox default. Recovery: detail page → CONTACT INFORMATION → pencil next to email → modal → tick "Is verified" → Save (do NOT use "Resend Code" — same broken SMTP path).

**Why this set is one bump and not three patches**: all three quirks have the same trigger pattern (manual UI provisioning of a human user) and the same cure pattern (operator checklist + retroactive fix in the user detail page). Ship them together as a "manual UI human-user provisioning" capability uplift.
