---
name: zitadel-idp
description: Helps integrate Zitadel as the OIDC identity provider in self-hosted projects, avoiding a known set of high-friction quirks (FirstInstance env placement, volume permissions, v1/v2 API split, tenant→orgId mapping, JWT validation traps, loginV2 instance flag, silent-renew redirect URI, JWT access token sem profile claims, JWKS over self-signed HTTPS, idempotent bootstrap "No changes" 400, TLS-terminating reverse proxy, secure-context PKCE, boot-time `signinSilent` recursion when `<AuthProvider>` wraps the silent-renew route, StrictMode + closure `cancelled` flag locking the SPA in "Verifying session…", `post_logout_redirect_uri invalid` from byte-mismatch, branding da Login UI v1 ignorado quando o projeto não tem `privateLabelingSetting`, paths v4 dos assets/textos diferentes do Zitadel v1/v2, 401 storm pós `--reset-zitadel` por processo dev runner velho com env+JWKS na heap, multi-app YAML refactor que ignora env vars OIDC_REDIRECT_URIS dinâmicas do dev.sh). Use this skill PROACTIVELY whenever the user is starting Zitadel integration, writing or debugging a docker-compose for Zitadel, building a bootstrap script that calls the Management API, configuring OIDC clients, validating Zitadel-issued JWTs in Node/TypeScript, applying brand identity (logo, colors, custom texts) to the hosted Login UI, or hitting 400/401/403/404 errors against Zitadel — including 404 na hosted UI (`/ui/v2/login/login`), 400 em silent renew, 401 com JWT aparentemente válido (incluindo 401 storm em LAN HTTPS atrás de proxy reverso), 401 storm que volta toda vez depois de `--reset-zitadel` mesmo com `.env` corrigido (processos `tsx watch`/`nodemon` órfãos com JWKS antiga em memória), UI gating que falha com role `battery.admin`, "Entrar não faz nada" sem erro no console, bootstrap idempotente que quebra com 400 `COMMAND-1m88i "No changes"` ou `Org-8nfSr "Private Label Policy has not been changed"`, label policy aplicada na org sem aparecer na tela de login (azul Zitadel default), 405 Method Not Allowed em `/assets/v1/orgs/me/policy/label/...`, 404 em `/management/v1/policies/custom_login_text/{lang}`, 400 `LANG-lg4DP "Language is not supported"` em `pt-BR`, F5 expulsando o usuário com `InMemoryWebStorage`, SPA preso em "Verifying session…" sem 400 visível (iframe recursion), ou logout que devolve `?error=invalid_request post_logout_redirect_uri invalid`, ou callback do Zitadel batendo em `redirect_uri missing in client configuration` após refatoração single-app→multi-app YAML que sobrescreve env vars dinâmicas do dev workflow. Triggers: "Zitadel", "self-hosted IdP", "bootstrap-zitadel", "@zitadel/node", "/management/v1", "/admin/v1", "FirstInstance", "OIDC PKCE", "validate Zitadel token", "x-zitadel-orgid", "loginV2", "silent renew 400", "Login UI v1 v2", "NODE_EXTRA_CA_CERTS", "createRemoteJWKSet self-signed", "tlsMode external", "COMMAND-1m88i", "crypto.subtle secure context", "signinRedirect silent fail", "boot-time signinSilent", "F5 perde sessão", "post_logout_redirect_uri invalid", "StrictMode signinSilent", "InMemoryWebStorage refresh", "iframe silent renew recursion", "LabelPolicy Zitadel", "PRIVATE_LABELING_SETTING", "Org-0K9dq", "Org-8nfSr", "LANG-lg4DP", "label policy não aparece", "login UI azul Zitadel", "branding Zitadel", "customizar tela de login Zitadel", "Powered by Zitadel disable", "tela piscando depois do login", "401 storm depois de --reset-zitadel", "tsx watch zumbi", "redirect_uri missing in client configuration", "multi-app YAML override env", "applications redirectUris", "bootstrap idempotente multi-app".
---
...
**Scope**: Zitadel `v4.x` (latest as of 2026-04). v3 differs in a few places (notably no `OrganizationService.AddOrganization` v2 endpoint) — the references flag breaking changes when relevant.
...
## The twenty-three quirks (the headline list)
