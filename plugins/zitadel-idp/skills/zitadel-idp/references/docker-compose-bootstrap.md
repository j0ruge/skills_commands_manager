...
- **`--tlsMode disabled` (or a stale container) breaks Zitadel's own admin Console with `[unknown] Failed to fetch`.** The Console's in-memory `environment.json` renders `api: http://…` while `issuer: https://…`; the HTTPS page then blocks the `http` api as **mixed content**. `issuer` comes from the persisted instance config (stays https); `api` is computed **per-request** and only renders https when the binary trusts the proxy's `X-Forwarded-Proto`, i.e. under `--tlsMode external`. Beware: `docker start`/`restart` reuses a container's **create-time** args — a container first built without the override stays on `--tlsMode disabled` forever; **recreate** it (`up -d --force-recreate zitadel`), don't `start` it. See `troubleshooting.md §"Zitadel admin Console: [unknown] Failed to fetch (environment.json api http vs issuer https)"` (Quirk 43).
